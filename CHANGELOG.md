...
- **World light blocks now illuminate passing Valkyrien Skies ships.** The last missing direction: a lamp post, glowstone, or any luminous block in the static world lights up a ship's hull as it sails past, dimmed by distance and respecting the ship's own geometry (line-of-sight in shipyard space). Performant by design: world lamps are static, so they're cached per chunk (shared by all ships, unaffected by ship movement, refreshed every 3 s) with a palette-level pre-filter that skips terrain sections containing no luminous block type without reading a single block; only the cached lamp positions are re-projected through the ship's current transform each tick, capped at the 16 strongest lamps per ship. With this, light now flows all four ways: entity→world, entity→ship, ship→world, and world→ship.
- **Ship-to-ship lighting.** A ship's lamps also illuminate other ships: each nearby ship's emitters are collected at their current world positions and fed into every other ship's world→ship projection, competing in the same distance ranking and per-ship cap as static world lamps. Two ships passing at night light each other's hulls; projected anchors are mod-recorded and excluded from emitter scans, so light can't cascade endlessly between ships.
